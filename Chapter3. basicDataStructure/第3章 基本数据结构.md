...
需求:<font color="red">**请设计一个函数,干函数可以将任意中序表达式转换为后序表达式.**</font>

- a. 该函数接收一个以<font color="red">**字符串形式**</font>表示的中序表达式.
- b. 该字符串以空格为分隔符
- c. 该中序表达式中仅含有`+ - * /`四种操作符
- e. 该中序表达式中可能含有`()`
- b. 该函数的返回值为该中序表达式的后序表达式
...
- step2. 分类讨论:
	...
	若该字符为<font color="red">**操作符,**</font>则继续分类讨论:
		...
		若此时栈不为空,则:
			...
			若此时栈内存在运算优先级高于或等于该运算符的运算符(例如A * B + C中遍历到+时的情况),则将栈内所有元素弹出,	 // TODO
			并按弹出顺序追加至结果列表尾部;然后将该运算符压入栈内.
			
			若此时栈内不存在运算优先级高于或等于该运算符的运算符(例如A + B * C中遍历到*时的情况),则将该运算符压入栈内, // TODO
			继续遍历下一个字符.
...
可以使用一个字典,该字典将每个运算符映射为一个整数,以该整数的大小表示运算符优先级的高低.
